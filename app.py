...
# BANCO DE DADOS HIERÁRQUICO (RESPEITANDO CATEGORIA -> TIPO -> CAMPEONATO)
DADOS_MUNDIAIS = {
    "🏆 COPA DO MUNDO 2026": {
        "Seleções FIFA": {
            "Principais": ["Brasil", "Argentina", "França", "Alemanha", "Espanha", "Inglaterra", "Portugal", "Holanda", "Itália", "Uruguai"],
            "Eliminatórias": ["Colômbia", "Equador", "Chile", "Paraguai", "Japão", "Coreia do Sul", "Marrocos", "Senegal"]
        }
    },
    "🇧🇷 BRASIL (LIGAS & COPAS)": {
        "Nacional": {
            "Série A": ["Flamengo", "Palmeiras", "Botafogo", "Atlético-MG", "São Paulo", "Cruzeiro", "Fortaleza", "Internacional", "Fluminense", "Grêmio"],
            "Série B": ["Santos", "Sport", "Coritiba", "Goiás", "Ceará", "Vila Nova", "América-MG", "Operário"],
    ...
    "🌍 AMÉRICA DO SUL (CONMEBOL)": {
        "Continentais": {
            ...
            "Sul-Americana": ["Racing", "Lanús", "Defensa y Justicia", "Ind. Medellín", "Libertad"]
        }
    },
    "🇪🇺 EUROPA (PRINCIPAIS LIGAS)": {
        "Inglaterra": {
            "Premier League": ["Manchester City", "Arsenal", "Liverpool", "Aston Villa", "Tottenham", "Chelsea", "Manchester United", "Newcastle"]
        },
        "Espanha": {
            "La Liga": ["Real Madrid", "Barcelona", "Atlético de Madrid", "Girona", "Athletic Bilbao", "Real Sociedad"]
        },
        "Alemanha": {
            "Bundesliga": ["Bayer Leverkusen", "Bayern de Munique", "Dortmund", "RB Leipzig", "Stuttgart"]
        },
        "UEFA": {
            "Champions League": ["Inter de Milão", "PSG", "Benfica", "Porto", "Milan", "Napoli"]
        }
    },
    "🇸🇦 ORIENTE MÉDIO & ÁSIA": {
        "Arábia Saudita": {
    ...
    "🇺🇸 AMÉRICA DO NORTE (MLS)": {
        "Estados Unidos": {
            "MLS Pro": ["Inter Miami", "LA Galaxy", "Columbus Crew", "LAFC", "Seattle Sounders"]
        }
    }
}
...
st.markdown("""
    ...
    * { -ms-overflow-style: none !important; scrollbar-width: none !important; }
    ...
    /* REFORÇO ZERO WHITE - INPUTS E SELECTS */
    ...
    .banca-title-banner {
        background-color: #003399 !important; padding: 15px 25px; border-radius: 5px; color: white !important;
        font-size: 24px; font-weight: 800; margin-bottom: 35px; display: flex; align-items: center; gap: 15px;
    }

    .history-card-box { 
        background: #161b22 !important; border: 1px solid #30363d !important; padding: 15px 25px !important; border-radius: 8px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; 
    }

    .footer-shield { position: fixed; bottom: 0; left: 0; width: 100%; background-color: #0d0d12; height: 25px; border-top: 1px solid #1e293b; display: flex; justify-content: space-between; align-items: center; padding: 0 20px; font-size: 9px; color: #475569; z-index: 999999; }
    
    /* AJUSTE LABELS DOS SELECTS PARA FICAREM DISCRETOS */
    label[data-testid="stWidgetLabel"] p {
        color: #64748b !important; font-size: 10px !important; text-transform: uppercase !important; font-weight: 700 !important; letter-spacing: 0.5px !important;
    }
    </style>
...
with st.sidebar:
    # APLICADO: Link funcional que reseta para a aba home
    st.markdown("""
...
# TELA 2: GESTÃO DE BANCA
    ...
    with col_display:
        ...
        with g8: 
            st.markdown(f"""
                <div class="highlight-card">
                    <div style="color:#64748b; font-size:9px; text-transform: uppercase; font-weight: 700;">SAÚDE BANCA</div>
                    <div style="color:{saude_color}; font-size:16px; font-weight:900; margin-top:10px;">{saude_label}</div>
                    <div style="background:#1e293b; height:4px; width:80%; border-radius:10px; margin:10px auto;">
                        <div style="background:#00d2ff; height:100%; width:100%;"></div>
                    </div>
                </div>
            """, unsafe_allow_html=True)

# TELA 3: SCANNER PRÉ-LIVE (ARMONIA VISUAL RESTAURADA)
elif st.session_state.aba_ativa == "analise":
    st.markdown("<h2 style='color:white;'>🎯 SCANNER PRÉ-LIVE</h2>", unsafe_allow_html=True)
    
    # LINHA 1: TRÊS COLUNAS CONFORME IMAGEM
    col_cat, col_tipo, col_camp = st.columns(3)
    
    with col_cat:
        cat_sel = st.selectbox("🌎 CATEGORIA", list(DADOS_MUNDIAIS.keys()))
    
    with col_tipo:
        tipos_disponiveis = list(DADOS_MUNDIAIS[cat_sel].keys())
        tipo_sel = st.selectbox("📂 TIPO", tipos_disponiveis)
        
    with col_camp:
        campeonatos_disponiveis = list(DADOS_MUNDIAIS[cat_sel][tipo_sel].keys())
        camp_sel = st.selectbox("🏆 CAMPEONATO", campeonatos_disponiveis)
    
    # LINHA 2: DUAS COLUNAS (MANDANTE E VISITANTE)
    times_atuais = DADOS_MUNDIAIS[cat_sel][tipo_sel][camp_sel]
    
    col_mand, col_vis = st.columns(2)
    with col_mand:
        mandante = st.selectbox("🏠 MANDANTE", times_atuais, index=0)
    with col_vis:
        # Tenta colocar o segundo time como visitante por padrão
        idx_vis = 1 if len(times_atuais) > 1 else 0
        visitante = st.selectbox("🚀 VISITANTE", times_atuais, index=idx_vis)
    
    # BOTÃO CENTRALIZADO
    if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True):
        if mandante == visitante:
            st.error("Selecione times diferentes para a análise.")
        else:
    ...
    if st.session_state.analise_bloqueada:
        ...
        st.markdown(f"<h3 style='color:#9d54ff; text-align:center;'>RESULTADO IA: {m['casa']} vs {m['fora']}</h3>", unsafe_allow_html=True)
...
# DEMAIS TELAS (HISTÓRICO, VENCEDORES, ETC)
