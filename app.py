...
# FIX: STABLE FRAGMENT RENDERING | ANTI-FLICKER ENGINE v2
# ==============================================================================
...
# 2. INJEÇÃO DE CSS ESTÁTICO (CARREGAMENTO ÚNICO NO TOPO)
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;700;900&display=swap');
    
    /* BLOQUEIO DE FUNDO TOTAL PARA EVITAR FLASH BRANCO */
...
# --- [FUNÇÃO FRAGMENTADA PARA CONTEÚDO DINÂMICO] ---
@st.fragment
def render_dynamic_content():
    if st.session_state.aba_ativa == "home":
        ...
        with g_col1:
            nova_banca = st.number_input("VALOR TOTAL (R$)", min_value=0.0, value=st.session_state.banca_total)
            if st.button("SALVAR CONFIGURAÇÃO"):
                st.session_state.banca_total = nova_banca
                st.success("BANCA ATUALIZADA!")
        with g_col2:
            st.session_state.stake_padrao = st.select_slider("RISCO %", options=[0.5, 1.0, 2.0, 3.0, 5.0, 10.0], value=st.session_state.stake_padrao)
        ...
        with l3: draw_card("PRESSÃO FORA", "ATAQUES: 4", 25)
