...
# ==============================================================================
# [PROTOCOLO DE MANUTENÇÃO v72.0 - INTEGRIDADE TOTAL 640+ LINHAS]
# DIRETRIZ 1: HEADER NA SIDEBAR (TRAVA DE CICLO)
# DIRETRIZ 2: MANTER TRANSLATE3D E BACKFACE-VISIBILITY (TRAVA DE GPU)
# DIRETRIZ 3: NAVEGAÇÃO APENAS POR SESSION_STATE (ESTABILIDADE)
# DIRETRIZ 4: ESTILIZAÇÃO PRIORITÁRIA (ZERO WHITE REFORÇADO)
# DIRETRIZ 5: CÓDIGO 100% ÍNTEGRO - LAYOUT IMUTÁVEL - SEM FUNDO BRANCO
# ==============================================================================
...
# Verificação de Big Data (Arquivo coletado pelo script baixar_big_data.py)
        ...
        try:
            ...
            for i, row in df_live.head(20).iterrows():
                novos_jogos.append({
                    ...
                    "CT": row.get('CARTÕES', 'LIVE +1.5'), "E": "RACE 7", "TM": "ALTO FLUXO", "CH": "PRESSÃO", "DF": "GOLEIRO OK"
                })
        except: pass
    if len(novos_jogos) < 20:
        times_live = [("Liverpool", "Everton"), ("Real Madrid", "Sevilla"), ("Napoli", "Lazio"), ("Boca", "River"), ("Palmeiras", "Santos"), ("Benfica", "Porto"), ("PSG", "Lyon"), ("Arsenal", "Spurs")]
...
st.markdown("""
    ...
    * { -ms-overflow-style: none !important; scrollbar-width: none !important; }
    ...
    div.stButton > button:not([data-testid="stSidebar"] *) { background: linear-gradient(90deg, #6d28d9 0%, #06b6d4 100%) !important; color: #ffffff !important; border: none !important; padding: 15px 20px !important; font-weight: 900 !important; text-transform: uppercase !important; letter-spacing: 1.2px !important; border-radius: 6px !important; width: 100% !important; box-shadow: 0 4px 15px rgba(109, 40, 217, 0.3) !important; margin-top: 10px !important; transform: translate3d(0,0,0); }
    ...
    st.markdown("""
        <div class="betano-header">
            <div class="header-left">
                ...
                <div class="nav-links"><div class="nav-item">APOSTAS ESPORTIVAS</div><div class="nav-item">APOSTAS AO VIVO</div><div class="nav-item">ASSERTIVIDADE IA</div></div>
            </div>
...
def draw_card(title, value, perc, color_footer="linear-gradient(90deg, #6d28d9, #06b6d4)"):
    st.markdown(f"""
        <div style="background: #11151a; border: 1px solid #1e293b; padding: 20px; border-radius: 8px; text-align: center; height: 155px; margin-bottom: 15px;">
...
# ==============================================================================
# 4. LÓGICA DE TELAS (APARÊNCIA IMUTÁVEL - 640+ LINHAS)
# ==============================================================================
...
if st.session_state.aba_ativa == "home":
    ...
    c1, c2 = st.columns(2)
    with c1: t_casa = st.selectbox("🏠 TIME DA CASA", ["Real Madrid", "Flamengo", "Man City", "Palmeiras", "Liverpool"])
    with c2: t_fora = st.selectbox("🚀 TIME DE FORA", ["Barcelona", "Vasco", "Arsenal", "Santos", "Everton"])
    if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True):
        st.session_state.analise_bloqueada = {"casa": t_casa, "fora": t_fora, "vencedor": "ALTA PROB.", "gols": "OVER 1.5", "stake_val": "R$ 10.00", "cantos": "9.5+", "btss": "SIM", "cartoes": "4.5+", "chutes": "8.5 p/g", "confia": "94.2%", "data": datetime.now().strftime("%H:%M")}
    ...
    except: st.info("Aguardando o processamento automático dos dados de assertividade.")
    ...
    if not st.session_state.historico_calls: st.info("Nenhuma operação registrada.")
        ...
        for row in rows_hist:
            ...
            for i, call in enumerate(row):
                with cols[i]:
                    st.markdown(f"""<div class="kpi-detailed-card"><div style="color:#06b6d4; font-size:10px; font-weight:900; margin-bottom:5px;">HORÁRIO: {call['data']}</div><div style="color:white; font-size:12px; font-weight:800; margin-bottom:12px; border-bottom:1px solid #1e293b; padding-bottom:5px;">{call['casa']} vs {call['fora']}</div><div class="kpi-stat">🏆 CALL: <b>{call.get('vencedor', 'N/A')}</b></div><div class="kpi-stat">⚽ GOLS: <b>{call.get('gols', 'N/A')}</b></div><div class="kpi-stat">IA CONF: <b>{call.get('confia', 'N/A')}</b></div><div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#9d54ff; font-size:11px; font-weight:900; text-align:center;">INVESTIDO: {call['stake_val']}</div></div>""", unsafe_allow_html=True)
...
st.markdown("""<div class="footer-shield"><div>STATUS: ● IA OPERACIONAL | v72.0</div><div>JARVIS PROTECT</div></div>""", unsafe_allow_html=True)
