...
# --- INICIALIZAÇÃO DE MEMÓRIA (CORREÇÃO DO ERRO ATTRIBUTEERROR) ---
...
# --- CARREGAMENTO DE DADOS (FORÇANDO O GITHUB SEM CACHE) ---
def carregar_dados_vivos():
    ...
    try:
        # Pega os dados ignorando o cache do navegador
        res_d = requests.get(f"{url_d}?v={datetime.now().timestamp()}", timeout=10)
        res_h = requests.get(url_h, timeout=10)
...
# 3. SIDEBAR E HEADER
with st.sidebar:
    st.markdown("""<div class="betano-header"><div class="logo-link">GESTOR IA</div><div style="color:white; font-size:10px; font-weight:600;">SISTEMA DE TRADING PRO</div></div><div style="height:65px;"></div>""", unsafe_allow_html=True)
...
# ==============================================================================
# 4. LÓGICA DE TELAS (RESTAURADO E BLINDADO)
# ==============================================================================
...
if st.session_state.aba_ativa == "home":
    ...
    t_casa = st.text_input("🏠 DIGITE O NOME DO TIME", "Flamengo")
    ...
    if st.session_state.analise_bloqueada:
        ...
        with r2: draw_card("PALPITE", "OVER 1.5", 90)
        ...
        if st.button("📥 SALVAR CALL"):
            st.session_state.historico_calls.append({"data": datetime.now().strftime("%H:%M"), "casa": a['casa'], "fora": "Rival"})
            st.toast("✅ SALVO!")
