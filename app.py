...
# --- CARREGAMENTO DE DADOS (CÉREBRO DO SISTEMA) ---
@st.cache_data
def carregar_arquivos_jarvis():
    try:
...
df_diario, df_hist = carregar_arquivos_jarvis()

# ==============================================================================
# LÓGICA DO BOT (BACK-END): MOTOR IA TURBO
# ==============================================================================

def analisar_confronto_ia(time_casa):
    """
    JARVIS ENGINE: Cruza qualquer time com o histórico real de 5 anos.
    """
    confianca = 75.0
    sugestao = "OVER 1.5"
    
    if df_hist is not None:
        # Busca inteligente no histórico (ignora erros de digitação)
        ...
        if not filtro.empty:
            ...
            confianca = round(taxa + 10, 1)
            if confianca > 95: sugestao = "OVER 2.5"
    
    return min(confianca, 98.4), sugestao
...
with st.sidebar:
    st.markdown("""<div class="betano-header"><div class="header-left"><a href="#" class="logo-link">GESTOR IA</a><div class="nav-links"><div class="nav-item">AO VIVO</div><div class="nav-item">ESTATÍSTICAS</div></div></div><div class="header-right"><div class="registrar-pill">REGISTRAR</div><div class="entrar-grad">ENTRAR</div></div></div><div style="height:65px;"></div>""", unsafe_allow_html=True) 
...
if st.session_state.aba_ativa == "home":
    ...
    if df_diario is not None:
        st.markdown("### 📋 JOGOS MONITORADOS HOJE")
        st.dataframe(df_diario, use_container_width=True)

elif st.session_state.aba_ativa == "analise":
    st.markdown("<h2 style='color:white;'>🎯 SCANNER PRÉ-LIVE</h2>", unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    with col1: s_pais = st.selectbox("🌎 REGIÃO", ["BRASIL", "INGLATERRA", "ESPANHA", "ALEMANHA"])
    with col2: s_liga = st.selectbox("📂 GRUPO", ["SÉRIE A", "PREMIER LEAGUE", "LA LIGA", "BUNDESLIGA"])
    with col3: s_time = st.text_input("🏠 TIME DA CASA (Digite o nome)", "Flamengo")
    
    if st.button("⚡ EXECUTAR ALGORITMO"):
        conf, palpite = analisar_confronto_ia(s_time)
        st.session_state.analise_bloqueada = {"casa": s_time, "conf": conf, "gols": palpite}
        
    if st.session_state.analise_bloqueada:
        m = st.session_state.analise_bloqueada
        st.markdown(f"<div style='background:rgba(0,255,136,0.1); border-left:5px solid #00ff88; padding:15px; border-radius:5px;'>🟢 SISTEMA JARVIS: ANÁLISE REAL CONCLUÍDA</div>", unsafe_allow_html=True)
        st.markdown(f"<h3 style='text-align:center; color:#9d54ff; margin-top:15px;'>{m['casa']} vs Rival</h3>", unsafe_allow_html=True)
        r1, r2, r3, r4 = st.columns(4)
        with r1: draw_card("CONFIANÇA IA", f"{m['conf']}%", int(m['conf']))
        with r2: draw_card("PALPITE GOLS", m['gols'], 90)
        ...
        if st.button("📥 SALVAR CALL NO HISTÓRICO", use_container_width=True):
            st.session_state.historico_calls.append({"data": datetime.now().strftime("%H:%M"), "casa": m['casa'], "fora": "Rival", "status": "PENDENTE"})
    ...
    for c in reversed(st.session_state.historico_calls):
        st.markdown(f"""<div class="history-card-box"><div style="color:white; font-weight:800;">[{c['data']}] {c['casa']} x {c['fora']} | {c['status']}</div></div>""", unsafe_allow_html=True)
