...
# --- CSS DE ALTA FIDELIDADE (PROTEÇÃO DE UI) ---
st.markdown("""
    <style>
    ...
    /* SIDEBAR ULTRA-SUBIDA (REGRAS MILIMÉTRICAS) */
    ...
    /* BOTÃO FERRAMENTA CÁPSULA COM SCANNER */
...
# --- BANCO DE DADOS GLOBAL (NÃO ALTERAR) ---
db_global = {
    ...
    "🌍 EUROPA (ALTO VALOR)": {
        ...
        "Segunda Divisão": ["Championship (Inglaterra 2)"],
        "Torneios UEFA": ["Champions League", "Europa League"]
    },
...
# --- DICIONÁRIO DE TIMES REAL ---
times_db = {
    "Série A": ["Palmeiras", "Flamengo", "Botafogo", "Fortaleza", "São Paulo", "Internacional", "Cruzeiro", "Bahia", "Vasco", "Atlético-MG", "Corinthians", "Grêmio"],
    "Série B": ["Santos", "Sport", "Mirassol", "Novorizontino", "Ceará", "Goiás"],
    "Premier League": ["Man City", "Arsenal", "Liverpool", "Chelsea", "Man United", "Tottenham"],
    ...
    "Primeira Liga": ["Benfica", "Sporting", "Porto", "Braga"]
}

# --- SIDEBAR (DESIGN ORIGINAL PRESERVADO) ---
with st.sidebar:
...
if st.session_state.app_state == "processar":
    ...
    if st.button("⚡ INICIAR ANÁLISE MILIMÉTRICA"):
        with st.status("MAGI IA: Analisando Big Data...", expanded=True) as s:
            time.sleep(1)
            st.write("🛰️ Sincronizando scouts de chutes, gols e internet...")
            time.sleep(1)
            s.update(label="ANÁLISE MILIMÉTRICA CONCLUÍDA!", state="complete")
        
        # --- EXIBIÇÃO DE RESULTADOS MILIMÉTRICOS (RESTAURADA) ---
        st.success(f"🤖 **RESULTADOS GIAE:** {casa} vs {fora}")
        r1, r2, r3, r4 = st.columns(4)
        with r1:
            st.metric("Vencedor", casa, "68%")
            st.write("**Prob. Empate:** 12%")
        with r2:
            st.metric("Gols HT", "Sim", "84%")
            st.write("**Gols FT:** +2.5")
        with r3:
            st.metric("Escanteios", "Over 10.5", "Alta")
            st.write("**Chutes ao Gol:** 6.4")
        with r4:
            st.metric("Assertividade", "94.2%", "IA Pro")
            st.write("**Cartões:** Over 3.5")
            
        st.markdown("---")
        st.subheader("📋 Scout por Tempo de Jogo")
        st.table({
            "Métrica IA": ["Posse de Bola", "Chutes Direção Gol", "Tiros de Meta", "Defesas Goleiro"],
            "1º Tempo": ["55%", "3.2", "4.0", "2.1"],
            "2º Tempo": ["58%", "4.1", "5.2", "3.0"],
            "Final Partida": ["56%", "7.3", "9.2", "5.1"]
        })

else:
    st.markdown("### 🤖 Cockpit de Comando Ativado")
    st.info("Configuração visual concluída. Selecione Processar Algoritmo para operar.")
