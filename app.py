...
# ==============================================================================
# [GIAE KERNEL SHIELD v57.0 - ABSOLUTE STATE PERSISTENCE]
# FIX: NESTED BUTTON LOGIC | 8-CARD GRID PRESERVATION | MASTER DATABASE RECOVERY
# ==============================================================================
...
if 'memoria_analise' not in st.session_state:
    st.session_state.memoria_analise = {}
if 'msg_sucesso' not in st.session_state:
    st.session_state.msg_sucesso = False

# --- [LOCK] BLOCO DE SEGURANÇA CSS (ESTRUTURA COMPLETA DAS IMAGENS) ---
st.markdown("""
    <style>
...
if st.session_state.aba_ativa == "home":
    st.markdown('<div class="news-ticker">● LIVE: IA OPERACIONAL ● HIERARQUIA v57.0 ATIVA ● JARVIS PROTECT</div>', unsafe_allow_html=True)
...
# --- [ABA] SCANNER PRÉ-LIVE ---
    ...
    # EXIBIÇÃO PERSISTENTE (CARBON LOCK)
    if st.session_state.analise_ativa:
        m = st.session_state.memoria_analise
        ...
        with r3: st.markdown(f'<div class="highlight-card"><div style="color:#64748b; font-size:9px;">STAKE</div><div style="color:#22c55e; font-size:18px; font-weight:900; margin-top:10px;">R$ {stake:,.2f}</div></div>', unsafe_allow_html=True)
        ...
        # BOTÃO SALVAR COM LÓGICA DE MEMÓRIA
        if st.button("📥 ENVIAR PARA HISTÓRICO"):
            st.session_state.historico_calls.append(st.session_state.memoria_analise)
...
# --- [ABA] HISTÓRICO ---
    ...
    if not st.session_state.historico_calls:
        ...
        for call in reversed(st.session_state.historico_calls):
            st.markdown(f'<div style="background:#11151a; padding:15px; border-radius:8px; margin-bottom:10px; border:1px solid #1e293b;">[{call["data"]}] {call["casa"]} x {call["fora"]} - {call["gols"]}</div>', unsafe_allow_html=True)
...
# --- FOOTER ---
st.markdown("""<div class="footer-shield"><div>STATUS: ● IA OPERACIONAL | v57.0 LOCKED</div><div>JARVIS PROTECT</div></div>""", unsafe_allow_html=True)
