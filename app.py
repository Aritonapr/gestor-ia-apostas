    ...
    if len(novos_jogos) < 12:
        ...
        for i in range(len(novos_jogos), 12):
            ...
            novos_jogos.append({
                ...
                "V": "72% (LIVE)", "G": "+0.5 GOLS", "CT": "4.5 (HT: 2 | FT: 2)",
                "E": "10.5 total", "TM": "18+ (HT: 9 | FT: 9)", "CH": "10+ (HT: 5 | FT: 5)", "DF": "8+ (GOLEIRO OK)"
            })
...
if st.session_state.aba_ativa == "home":
    ...
    c1, c2 = st.columns(2)
    with c1: t_casa = st.selectbox("🏠 TIME DA CASA", ["Flamengo", "Real Madrid", "Man City"])
    with c2: t_fora = st.selectbox("🚀 TIME DE FORA", ["Palmeiras", "Barcelona", "Bayern"])
    ...
    for row in rows:
        ...
        for i, j in enumerate(row):
            with cols[i]:
                # EQUALIZAÇÃO PARA 7 RESULTADOS IGUAL AO BILHETE OURO PARA MANTER SIMETRIA
                st.markdown(f"""
                <div class="kpi-detailed-card">
