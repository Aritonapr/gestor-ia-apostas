...
# ==============================================================================
# [GIAE KERNEL SHIELD v44.0 - PROTOCOLO JARVIS SUPREME]
# FIX: EFEITO EXCLUSIVO NO BOTÃO DE ANÁLISE | SELECTS ULTRA-DARK
# ==============================================================================
...
st.markdown("""
    ...
    /* [02] SIDEBAR LOCK (320PX) - SEM EFEITOS DE BOTÃO */
    ...
    .nav-items span { color: #ffffff; font-size: 10px !important; text-transform: uppercase; margin-right: 15px; opacity: 0.7; }
    ...
    /* [04] REMOÇÃO DE FUNDO BRANCO EM SELECTS */
    ...
    /* [05] EFEITO EXCLUSIVO: BOTÃO RESULTADO ALGORITIMO (ZONA CENTRAL) */
    [data-testid="stMainBlockContainer"] div.stButton > button {
        ...
        color: white !important; font-weight: 900 !important; padding: 15px 40px !important; width: 100% !important;
    }
    [data-testid="stMainBlockContainer"] div.stButton > button:hover { transform: scale(1.01); filter: brightness(1.2); }

    @keyframes gradient-anim { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
    @keyframes pulse-glow { 0% { box-shadow: 0 0 5px rgba(109, 40, 217, 0.4); } 50% { box-shadow: 0 0 20px rgba(6, 182, 212, 0.7); } 100% { box-shadow: 0 0 5px rgba(109, 40, 217, 0.4); } }
...
if st.session_state.aba_ativa == "home":
    ...
    for col in [h1, h2, h3, h4]:
        with col: st.markdown('<div class="highlight-card"><div style="color:#64748b; font-size:9px;">LIVE</div><div style="color:white; font-size:16px; font-weight:900; margin-top:10px;">PROGNÓSTICO ATIVO</div><div class="conf-bar-bg"><div class="conf-bar-fill" style="width:80%;"></div></div></div>', unsafe_allow_html=True)
    
    st.markdown('<div style="height:10px;"></div>', unsafe_allow_html=True)
    
    h5, h6, h7, h8 = st.columns(4)
    for col in [h5, h6, h7, h8]:
        with col: st.markdown('<div class="highlight-card"><div style="color:#64748b; font-size:9px;">MÉTRICA</div><div style="color:white; font-size:16px; font-weight:900; margin-top:10px;">ANÁLISE DE TENDÊNCIA</div><div class="conf-bar-bg"><div class="conf-bar-fill" style="width:92%;"></div></div></div>', unsafe_allow_html=True)
...
# --- ANÁLISE MÉTRICA ---
    ...
    with c3: camp = st.selectbox("🏆 CAMPEONATO", list(DADOS_HIEARARQUIA[pais][tipo].keys()))
    ...
    if st.session_state.analise_pronta:
        ...
        for col in [r1, r2, r3, r4]:
            with col: st.markdown('<div class="highlight-card"><div style="color:#64748b; font-size:9px;">MÉTRICA IA</div><div style="color:white; font-size:14px; font-weight:900; margin-top:10px;">DADO DETECTADO</div><div class="conf-bar-bg"><div class="conf-bar-fill" style="width:75%;"></div></div></div>', unsafe_allow_html=True)
        st.markdown('<div style="height:10px;"></div>', unsafe_allow_html=True)
        r5, r6, r7, r8 = st.columns(4)
        for col in [r5, r6, r7, r8]:
            with col: st.markdown('<div class="highlight-card"><div style="color:#64748b; font-size:9px;">MÉTRICA IA</div><div style="color:white; font-size:14px; font-weight:900; margin-top:10px;">DADO DETECTADO</div><div class="conf-bar-bg"><div class="conf-bar-fill" style="width:85%;"></div></div></div>', unsafe_allow_html=True)

# --- [LOCK] FOOTER ---
st.markdown("""<div class="footer-shield"><div>STATUS: ● IA OPERACIONAL | KEY: GIAE-V17-ELITE-RECOVERY</div><div>GESTOR IA PRO v44.0 | JARVIS PROTECT</div></div>""", unsafe_allow_html=True)
