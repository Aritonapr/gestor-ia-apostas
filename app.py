...
if st.session_state.aba_ativa == "home":
    ...
    lista_base = ["Flamengo", "Palmeiras", "Real Madrid", "Barcelona", "Man City", "Liverpool", "Inter Miami", "River Plate", "Boca Juniors"]
    c1, c2 = st.columns(2)
    ...
    if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True):
        ...
        st.session_state.analise_bloqueada = {
            ...
            "chutes": "8.5 p/g", "confia": "94.2%", "data": datetime.now().strftime("%d/%m %H:%M"),
            "luz": "🟢", "motivo": "INFORMAÇÃO REAL", "cor": "#00ff88"
        }
    
    if st.session_state.analise_bloqueada:
        m = st.session_state.analise_bloqueada
        st.markdown(f"<div style='background: rgba(255,255,255,0.03); border-left: 5px solid {m['cor']}; padding: 18px; border-radius: 6px; margin-top: 25px;'><span style='font-size: 20px;'>{m['luz']}</span> <b style='color: white; margin-left: 15px;'>SISTEMA JARVIS:</b> <span style='color: {m['cor']}; font-weight: 800;'>{m['motivo']}</span></div>", unsafe_allow_html=True)
        st.markdown(f"<h3 style='color:white; text-align:center; margin-bottom: 30px;'>{m['casa']} vs {m['fora']}</h3>", unsafe_allow_html=True)
    ...
    with col_display:
        ...
        with g3: draw_card("STOP LOSS", f"R$ {v_loss:,.2f}", 100)
    ...
    for row in rows:
        ...
        for i, j in enumerate(row):
            with cols[i]:
                st.markdown(f"""
                <div class="kpi-detailed-card">
                    <div style="color:#ffcc00; font-size:10px; font-weight:900; margin-bottom:5px;">CHANCE VITÓRIA: {j['P']}</div>
                    <div style="color:white; font-size:12px; font-weight:800; margin-bottom:12px; border-bottom:1px solid #1e293b; padding-bottom:5px;">{j['C']} vs {j['F']}</div>
                    <div class="kpi-stat">🏆 VENCEDOR: <b>{j['V']}</b></div>
                    <div class="kpi-stat">⚽ GOLS: <b>{j['G']}</b></div>
                    <div class="kpi-stat">🟨 CARTÕES: <b>{j['CT']}</b></div>
                    ...
                    <div class="kpi-stat">🥅 CHUTES GOL: <b>{j['CH']}</b></div>
                    <div class="kpi-stat">🧤 DEFESAS: <b>{j['DF']}</b></div>
                    <div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#ffcc00; font-size:11px; font-weight:900; text-align:center;">
                        INVESTIMENTO: R$ {v_entrada:,.2f}
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        for i, j in enumerate(row):
            with cols[i]:
                ...
                <div class="kpi-detailed-card">
                    ...
                    <div class="kpi-stat">🥅 CHUTES GOL: <b>{j['CH']}</b></div>
                    <div class="kpi-stat">🧤 DEFESAS: <b>{j['DF']}</b></div>
                    <div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#00d2ff; font-size:11px; font-weight:900; text-align:center;">
                        INVESTIMENTO GOLS: R$ {v_entrada:,.2f}
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    for row in rows:
        ...
        for i, j in enumerate(row):
            with cols[i]:
                st.markdown(f"""
                <div class="kpi-detailed-card">
                    <div style="color:#ff4b4b; font-size:10px; font-weight:900; margin-bottom:5px;">PROB. ESCANTEIOS: {j['P']}</div>
                    <div style="color:white; font-size:12px; font-weight:800; margin-bottom:12px; border-bottom:1px solid #1e293b; padding-bottom:5px;">{j['C']} vs {j['F']}</div>
                    <div class="kpi-stat">🏆 VENCEDOR: <b>{j['V']}</b></div>
                    <div class="kpi-stat">⚽ GOLS: <b>{j['G']}</b></div>
                    <div class="kpi-stat">🟨 CARTÕES: <b>{j['CT']}</b></div>
                    ...
                    <div class="kpi-stat">🥅 CHUTES GOL: <b>{j['CH']}</b></div>
                    <div class="kpi-stat">🧤 DEFESAS: <b>{j['DF']}</b></div>
                    <div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#ff4b4b; font-size:11px; font-weight:900; text-align:center;">
                        INVESTIMENTO CANTOS: R$ {v_entrada:,.2f}
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    if not st.session_state.historico_calls:
        ...
        # Mostra o histórico usando KPI Cards
        rows = [st.session_state.historico_calls[i:i + 4] for i in range(0, len(st.session_state.historico_calls), 4)]
        for row in reversed(rows): # Mais recentes primeiro
            cols = st.columns(4)
            for i, call in enumerate(row):
                with cols[i]:
                    st.markdown(f"""
                    <div class="kpi-detailed-card">
                        <div style="color:#9d54ff; font-size:10px; font-weight:900; margin-bottom:5px;">DATA: {call['data']}</div>
                        ...
                        <div class="kpi-stat">🏆 VENCEDOR: <b>{call['vencedor']}</b></div>
                        <div class="kpi-stat">⚽ GOLS: <b>{call['gols']}</b></div>
                        <div class="kpi-stat">🟨 CARTÕES: <b>{call['cartoes']}</b></div>
                        <div class="kpi-stat">🚩 ESCANTEIOS: <b>{call['cantos']}</b></div>
                        <div class="kpi-stat">👟 TIROS META: <b>{call['chutes']}</b></div>
                        <div class="kpi-stat">🥅 CHUTES GOL: <b>{call['btss']}</b></div>
                        <div class="kpi-stat">📊 IA CONFIANÇA: <b>{call['confia']}</b></div>
                        <div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#06b6d4; font-size:11px; font-weight:900; text-align:center;">
                            STAKE SALVA: {call['stake_val']}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
