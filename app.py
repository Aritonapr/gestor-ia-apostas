    ...
    st.markdown("""
        <div class="betano-header">
            <div class="header-left">
                ...
                <div class="nav-links">
                    <div class="nav-item">APOSTAS ESPORTIVAS</div><div class="nav-item">APOSTAS AO VIVO</div><div class="nav-item">ESTATÍSTICAS AVANÇADAS</div>
                </div>
            </div>
...
# ==============================================================================
# 4. LÓGICA DE TELAS (20 TIMES EM TUDO + GESTÃO CORRIGIDA)
# ==============================================================================
...
if st.session_state.aba_ativa == "home":
    ...
    with col_display:
        ...
        with r1: draw_card("VALOR ENTRADA", f"R$ {v_stake:,.2f}", 100, "#00d2ff")
        with r2: draw_card("STOP GAIN (R$)", f"R$ {v_meta:,.2f}", 100, "#00d2ff")
        with r3: draw_card("STOP LOSS (R$)", f"R$ {v_loss:,.2f}", 100, "#00d2ff")
        with r4: draw_card("ALVO FINAL", f"R$ {st.session_state.banca_total + v_meta:,.2f}", 100, "#00d2ff")
        r5, r6, r7, r8 = st.columns(4)
        with r5: draw_card("RISCO TOTAL", f"{st.session_state.stake_padrao}%", 100, "#00d2ff")
        with r6: draw_card("ENTRADAS/META", f"{int(v_meta/v_stake) if v_stake > 0 else 0}", 100, "#00d2ff")
        with r7: draw_card("ENTRADAS/LOSS", f"{int(v_loss/v_stake) if v_stake > 0 else 0}", 100, "#00d2ff")
        with r8: draw_card("SAÚDE BANCA", saude_label, 100, saude_color)
    ...
    for row in rows:
        ...
        for i, j in enumerate(row):
            with cols[i]:
                st.markdown(f"""<div class="kpi-detailed-card"><div style="color:#00ff88; font-size:10px; font-weight:900; margin-bottom:5px;">IA LIVE: {j['P']}</div><div style="color:white; font-size:12px; font-weight:800; margin-bottom:12px; border-bottom:1px solid #1e293b; padding-bottom:5px;">{j['C']} vs {j['F']}</div><div class="kpi-stat">🏆 VENCEDOR: <b>{j['V']}</b></div><div class="kpi-stat">⚽ GOLS: <b>{j['G']}</b></div><div class="kpi-stat">🟨 CARTÕES: <b>{j.get('CT', '4.5 total')}</b></div><div class="kpi-stat">🚩 ESCANTEIOS: <b>{j.get('E', '9.5 total')}</b></div><div class="kpi-stat">👟 TIROS META: <b>{j.get('TM', '14+ total')}</b></div><div class="kpi-stat">🥅 CHUTES GOL: <b>{j.get('CH', '9+ total')}</b></div><div class="kpi-stat">🧤 DEFESAS: <b>{j.get('DF', '7+ total')}</b></div><div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#9d54ff; font-size:11px; font-weight:900; text-align:center;">INVESTIMENTO LIVE: R$ {v_entrada:,.2f}</div></div>""", unsafe_allow_html=True)
    ...
    for row in rows:
        ...
        for i, j in enumerate(row):
            with cols[i]:
                st.markdown(f"""<div class="kpi-detailed-card"><div style="color:#ffcc00; font-size:10px; font-weight:900; margin-bottom:5px;">CHANCE VITÓRIA: {j['P']}</div><div style="color:white; font-size:12px; font-weight:800; margin-bottom:12px; border-bottom:1px solid #1e293b; padding-bottom:5px;">{j['C']} vs {j['F']}</div><div class="kpi-stat">🏆 VENCEDOR: <b>{j['V']}</b></div><div class="kpi-stat">⚽ GOLS: <b>{j['G']}</b></div><div class="kpi-stat">🟨 CARTÕES: <b>{j['CT']}</b></div><div class="kpi-stat">🚩 ESCANTEIOS: <b>{j['E']}</b></div><div class="kpi-stat">👟 TIROS META: <b>{j['TM']}</b></div><div class="kpi-stat">🥅 CHUTES GOL: <b>{j['CH']}</b></div><div class="kpi-stat">🧤 DEFESAS: <b>{j['DF']}</b></div><div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#ffcc00; font-size:11px; font-weight:900; text-align:center;">INVESTIMENTO: R$ {v_entrada:,.2f}</div></div>""", unsafe_allow_html=True)
    ...
    for row in rows:
        ...
        for i, j in enumerate(row):
            with cols[i]:
                st.markdown(f"""<div class="kpi-detailed-card"><div style="color:#ff4b4b; font-size:10px; font-weight:900; margin-bottom:5px;">PROB. ESCANTEIOS: {j['P']}</div><div style="color:white; font-size:12px; font-weight:800; margin-bottom:12px; border-bottom:1px solid #1e293b; padding-bottom:5px;">{j['C']} vs {j['F']}</div><div class="kpi-stat">🏆 VENCEDOR: <b>{j['V']}</b></div><div class="kpi-stat">⚽ GOLS: <b>{j['G']}</b></div><div class="kpi-stat">🟨 CARTÕES: <b>{j['CT']}</b></div><div class="kpi-stat">🚩 ESCANTEIOS: <b>{j['E']}</b></div><div class="kpi-stat">👟 TIROS META: <b>{j['TM']}</b></div><div class="kpi-stat">🥅 CHUTES GOL: <b>{j['CH']}</b></div><div class="kpi-stat">🧤 DEFESAS: <b>{j['DF']}</b></div><div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#ff4b4b; font-size:11px; font-weight:900; text-align:center;">INVESTIMENTO: R$ {v_entrada:,.2f}</div></div>""", unsafe_allow_html=True)
