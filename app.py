...
# --- 2. ESTILO VISUAL BLINDADO (ESTRUTURA v12 + SIDEBAR COMPACTA) ---
st.markdown("""
    <style>
    ...
    [data-testid="stSidebar"] { background-color: #111a21; border-right: 2px solid #f05a22; min-width: 240px !important; }
    ...
    .stButton > button {
        ...
        font-weight: bold !important; width: 100% !important; height: 26px !important; /* Altura mínima para caber tudo */
    ...
    .cat-label { color: #5a6b79; font-size: 8px; font-weight: bold; margin-top: 5px; text-transform: uppercase; border-left: 2px solid #f05a22; padding-left: 5px; margin-bottom: 2px; }

    /* BOTÃO DE ALERTA PISCANTE */
    .stButton > button[key="alerta_principal"] {
        background-color: #f05a22 !important; color: white !important; height: 50px !important;
        font-family: 'Orbitron', sans-serif !important; font-size: 14px !important;
        border-radius: 10px !important; animation: pulse-orange 2s infinite !important; font-weight: 900 !important;
    }
    @keyframes pulse-orange { 0% { box-shadow: 0 0 0 0 rgba(240,90,34,0.7); } 70% { box-shadow: 0 0 0 15px rgba(240,90,34,0); } 100% { box-shadow: 0 0 0 0 rgba(240,90,34,0); } }
...
# --- 3. LÓGICA DE NAVEGAÇÃO E DADOS ---
if 'tela' not in st.session_state: st.session_state.update(tela='radar', liga='BRA_A', nome='SÉRIE A - BRASILEIRÃO')
...
def load_data(liga):
    ...
    try:
        ...
        return pd.DataFrame([['Botafogo', 'Flamengo', 1, 0], ['Chelsea', 'Arsenal', 2, 2]], columns=['HomeTeam', 'AwayTeam', 'FTHG', 'FTAG'])

# --- 4. SIDEBAR REESTRUTURADA (TUDO VISÍVEL) ---
with st.sidebar:
...
# --- 5. ÁREA PRINCIPAL (BOTÃO DE ALERTA ATIVO) ---
if st.button("⚠️ APOSTAS ENCONTRADAS IA: 12 OPORTUNIDADES DISPONÍVEIS", key="alerta_principal"):
    st.session_state.tela = 'scanner'

# Navegação por botões (Estável)
n1, n2, n3 = st.columns(3)
with n1: 
    if st.button("🎯 RADAR NEURAL"): st.session_state.tela = 'radar'
with n2:
    if st.button("🔍 SCANNER DIÁRIO"): st.session_state.tela = 'scanner'
with n3:
    if st.button("💰 GESTÃO BANCA"): st.session_state.tela = 'gestao'

df = load_data(st.session_state.liga)
times = sorted(df['HomeTeam'].unique())

if st.session_state.tela == 'radar':
    exec_btn = st.button("🔥 EXECUTAR ALGORITMO COMPLETO")
    c1, c2 = st.columns(2)
    with c1: t_casa = st.selectbox("Mandante", times, key="sel_c")
    with c2: t_fora = st.selectbox("Visitante", times, index=min(1, len(times)-1), key="sel_f")
...
elif st.session_state.tela == 'scanner':
    st.markdown(f"### 🔍 Scanner de Oportunidades: {st.session_state.nome}")
    for i in range(3):
        with st.expander(f"💰 JOGO COM VALOR: {times[i]} x {times[-(i+1)]}"):
            st.write("A IA sugere entrada em Over Gols e Vitória do Mandante.")

elif st.session_state.tela == 'gestao':
    st.write("### Calculadora de Stake")
    st.number_input("Banca R$", value=1000)

st.markdown("<br><p style='text-align:center; opacity:0.3; font-size:9px;'>GESTOR IA v14.1 - SISTEMA REORGANIZADO</p>", unsafe_allow_html=True)
