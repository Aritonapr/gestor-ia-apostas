...
# REPARO: ESTABILIZAÇÃO TOTAL DO HEADER (ANTI-FLICKER)
# ==============================================================================
...
# 2. BLOCO ÚNICO DE CSS E HEADER (FORA DE QUALQUER LÓGICA PARA NÃO PISCAR)
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;700;900&display=swap');
    
    /* BLOQUEIO DE FUNDO PARA EVITAR FLASH BRANCO */
...
with st.sidebar:
    ...
    if st.button("🎯 SCANNER PRÉ-LIVE", key="btn_nav_pre"): st.session_state.aba_ativa = "analise"
    if st.button("📡 SCANNER EM TEMPO REAL", key="btn_nav_live"): st.session_state.aba_ativa = "live"
    if st.button("💰 GESTÃO DE BANCA", key="btn_nav_gestao"): st.session_state.aba_ativa = "gestao"
    if st.button("📜 HISTÓRICO DE CALLS", key="btn_nav_hist"): st.session_state.aba_ativa = "historico"
...
if st.session_state.aba_ativa == "home":
    ...
    cat = c1.selectbox("🌎 CATEGORIA", list(DADOS_HIEARARQUIA.keys()), key="sel_categoria")
    tip = c2.selectbox("📂 TIPO", list(DADOS_HIEARARQUIA[cat].keys()), key="sel_tipo")
    cmp = c3.selectbox("🏆 CAMPEONATO", list(DADOS_HIEARARQUIA[cat][tip].keys()), key="sel_campeonato")
    t1, t2 = st.columns(2)
    ...
    if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True, key="btn_executar"):
    ...
    if st.session_state.analise_bloqueada:
        ...
        if st.button("📥 SALVAR CALL NO HISTÓRICO", use_container_width=True, key="btn_salvar_hist"):
    ...
    with g_col1:
        st.session_state.banca_total = st.number_input("VALOR TOTAL DA BANCA (R$)", min_value=0.0, value=st.session_state.banca_total, key="num_banca")
        st.success(f"Banca atual: R$ {st.session_state.banca_total:,.2f}")
    with g_col2:
        st.session_state.stake_padrao = st.select_slider("DEFINIR % DE RISCO", options=[0.5, 1.0, 2.0, 3.0, 5.0, 10.0], value=st.session_state.stake_padrao, key="sli_stake")
