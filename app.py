...
# DICA: Para adicionar novos times, basta seguir o padrão abaixo.
DADOS_HIEARARQUIA = {
    "BRASIL": {
        "Nacional": {
            "Brasileirão Série A": ["Flamengo", "Palmeiras", "Botafogo", "Fortaleza", "São Paulo", "Internacional"],
            "Brasileirão Série B": ["Santos", "Sport", "Ceará", "Goiás"]
        },
        "Estadual": {
            "Paulistão": ["Palmeiras", "São Paulo", "Corinthians", "Santos", "Bragantino"],
    ...
    "INGLATERRA": {
        "Nacional": {
            ...
            "Championship": ["Leeds", "Leicester", "Ipswich"]
        }
    },
    "ESPANHA": {
        "Nacional": {
...
# ==============================================================================
# [CONFIGURAÇÃO DA PÁGINA] - NÃO ALTERAR
# ==============================================================================
st.set_page_config(
...
# ==============================================================================
# [BLOCO DE ESTILO CSS] - NÃO ALTERAR (PROTEGE O VISUAL)
# ==============================================================================
st.markdown("""
    <style>
    ...
    .logo-link { color: #9d54ff !important; font-weight: 900; font-size: 20px !important; text-transform: uppercase; letter-spacing: 1px; margin-right: 40px; text-decoration: none !important; cursor: pointer !important; }
...
# ==============================================================================
    ...
    # Seleção Dinâmica baseada nos Dados do topo
    col1, col2, col3 = st.columns(3)
    with col1: 
        pais_sel = st.selectbox("🌎 CATEGORIA", list(DADOS_HIEARARQUIA.keys()))
    with col2: 
        tipo_sel = st.selectbox("📂 TIPO", list(DADOS_HIEARARQUIA[pais_sel].keys()))
    with col3: 
        camp_sel = st.selectbox("🏆 CAMPEONATO", list(DADOS_HIEARARQUIA[pais_sel][tipo_sel].keys()))
    
    t1, t2 = st.columns(2)
    lista_times = DADOS_HIEARARQUIA[pais_sel][tipo_sel][camp_sel]
    with t1: 
        casa = st.selectbox("🏠 CASA", lista_times)
    with t2: 
        fora = st.selectbox("🚀 VISITANTE", [t for t in lista_times if t != casa])
        ...
        st.markdown(f'''
            <div style="background:#1a202c; border: 1px solid #6d28d9; padding:25px; border-radius:10px; margin-top:20px;">
                <div style="color:#9d54ff; font-weight:900; font-size:18px; margin-bottom:15px; text-transform: uppercase;">
                    RESULTADO ALGORITIMO: {casa} vs {fora}
                </div>
        ''', unsafe_allow_html=True)
        r1, r2 = st.columns(2)
        with r1:
