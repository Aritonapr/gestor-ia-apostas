...
# ==============================================================================
# [GIAE KERNEL SHIELD v45.0 - PROTOCOLO JARVIS SUPREME]
# ESTADO: SIMETRIA TOTAL | SIDEBAR SEM SCROLL | BOTÕES AJUSTADOS
# ==============================================================================
...
st.markdown("""
    ...
    /* [02] SIDEBAR - SEM BARRA DE ROLAGEM E SIMETRIA */
    ...
    .logo-link { color: #9d54ff !important; font-weight: 900; font-size: 20px !important; text-transform: uppercase; text-decoration: none !important; transition: 0.3s; margin-right: 40px; }
    ...
    .registrar-pill { color: #ffffff !important; font-size: 10px !important; font-weight: 700 !important; border: 1px solid #ffffff !important; padding: 6px 15px !important; border-radius: 20px !important; transition: 0.3s; }
    ...
    /* [04] BOTÃO EXECUTAR ALGORITIMO - AJUSTADO AO TEXTO */
...
st.markdown(f"""
    <div class="betano-header">
        <div class="header-left">
            ...
            <div class="nav-items">
                <span>Apostas Esportivas</span><span>Apostas ao Vivo</span><span>Apostas Encontradas</span><span>Estatísticas</span>
            </div>
        </div>
...
if st.session_state.aba_ativa == "home":
    ...
    for col in [h1, h2, h3, h4]:
        with col: st.markdown('<div class="highlight-card"><div style="color:#64748b; font-size:9px;">LIVE</div><div style="color:white; font-size:16px; font-weight:900; margin-top:10px;">ANÁLISE ATIVA</div><div class="conf-bar-bg"><div class="conf-bar-fill" style="width:80%;"></div></div></div>', unsafe_allow_html=True)
    st.markdown('<div style="height:10px;"></div>', unsafe_allow_html=True)
    h5, h6, h7, h8 = st.columns(4)
    for col in [h5, h6, h7, h8]:
        with col: st.markdown('<div class="highlight-card"><div style="color:#64748b; font-size:9px;">ESTATÍSTICA</div><div style="color:white; font-size:16px; font-weight:900; margin-top:10px;">TENDÊNCIA IA</div><div class="conf-bar-bg"><div class="conf-bar-fill" style="width:92%;"></div></div></div>', unsafe_allow_html=True)
...
# --- ANÁLISE MÉTRICA ---
    ...
    with c1: pais = st.selectbox("🌎 CATEGORIA", list(DADOS_HIEARARQUIA.keys()))
    with c2: tipo = st.selectbox("📂 TIPO", list(DADOS_HIEARARQUIA[pais].keys()))
    with col3 if 'col3' in locals() else c3: camp = st.selectbox("🏆 CAMPEONATO", list(DADOS_HIEARARQUIA[pais][tipo].keys()))
    ...
    if st.session_state.analise_pronta:
        ...
        for col in [r1, r2, r3, r4]:
            with col: st.markdown('<div class="highlight-card"><div style="color:#64748b; font-size:9px;">DADO IA</div><div style="color:white; font-size:14px; font-weight:900; margin-top:10px;">METRICA REAL</div><div class="conf-bar-bg"><div class="conf-bar-fill" style="width:75%;"></div></div></div>', unsafe_allow_html=True)
        st.markdown('<div style="height:10px;"></div>', unsafe_allow_html=True)
        r5, r6, r7, r8 = st.columns(4)
        for col in [r5, r6, r7, r8]:
            with col: st.markdown('<div class="highlight-card"><div style="color:#64748b; font-size:9px;">DADO IA</div><div style="color:white; font-size:14px; font-weight:900; margin-top:10px;">METRICA REAL</div><div class="conf-bar-bg"><div class="conf-bar-fill" style="width:85%;"></div></div></div>', unsafe_allow_html=True)

# --- [LOCK] FOOTER ---
st.markdown("""<div class="footer-shield"><div>STATUS: ● IA OPERACIONAL | KEY: GIAE-V17-ELITE-RECOVERY</div><div>GESTOR IA PRO v45.0 | JARVIS PROTECT</div></div>""", unsafe_allow_html=True)
