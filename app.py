...
# ==============================================================================
# [PROTOCOLO DE MANUTENÇÃO v58.12 - RESTAURAÇÃO INTEGRAL PIT]
...
if st.session_state.aba_ativa == "home":
    ...
    with h8: draw_card("SISTEMA", "JARVIS v58.12", 100)
...
# TELA 3: SCANNER PRÉ-LIVE (RESTAURAÇÃO TOTAL v58.12)
elif st.session_state.aba_ativa == "analise":
    st.markdown("<h2 style='color:white;'>🎯 SCANNER PRÉ-LIVE</h2>", unsafe_allow_html=True)
    
    # [ESTRUTURA DE DADOS ORIGINAL RESTAURADA]
    db_paises = {
    ...
    if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True):
        ...
        # Salvando com todos os campos necessários para evitar o KeyError
    ...
    # EXIBIÇÃO COM PROTEÇÃO DE CAMPO (.get)
    if st.session_state.analise_bloqueada:
        ...
        conf_label = m.get('conf', '90%')
        with r5: draw_card("IA CONF.", conf_label, int(conf_label.replace('%','')))
        with r6: draw_card("PRESSÃO", "ALTA", 88)
        with r7: draw_card("TENDÊNCIA", "SUBINDO", 60)
        with r8: draw_card("SISTEMA", "v58.12", 100)
    ...
    if not st.session_state.historico_calls: st.info("Nenhuma operação registrada.")
        ...
        for i, call in enumerate(reversed(st.session_state.historico_calls)):
            ...
            with col_info: st.markdown(f"""<div class="history-card-box"><div style="color:white; font-weight:800;"><span style="color:#9d54ff;">[{call.get('data', '??:??')}]</span> {call.get('casa','Time A')} x {call.get('fora','Time B')} <span style="color:#06b6d4; margin-left:20px;">{call.get('stake_val','R$ 0')} | {call.get('gols','-')}</span></div></div>""", unsafe_allow_html=True)
            with col_del:
...
st.markdown("""<div class="footer-shield"><div>STATUS: ● IA OPERACIONAL | v58.12</div><div>JARVIS PROTECT</div></div>""", unsafe_allow_html=True)
