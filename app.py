...
# 2. [CAMADA DE PROTEÇÃO 1] - CSS DE PERSISTÊNCIA E EFEITOS DE BOTÃO
st.markdown("""
    <style>
    ...
    /* EFEITO NOS BOTÕES EXECUTAR E SALVAR (MAIN AREA) */
...
with st.sidebar:
    ...
    st.button("🏆 COMPETIÇÕES")

# --- [DADOS INTEGRAIS RESTAURADOS] ---
DADOS_HIEARARQUIA = {
    "🏆 COPA DO MUNDO 2026": {"Seleções FIFA": {"Principais": ["Brasil", "Argentina", "França", "Alemanha", "Espanha", "Portugal"]}},
    "🇧🇷 BRASIL (LIGAS & COPAS)": {"Campeonato Brasileiro": {"Série A": ["Flamengo", "Palmeiras", "Botafogo", "São Paulo", "Atlético-MG", "Grêmio", "Corinthians", "Vasco"]}},
    "🇪🇺 EUROPA": {"Ligas": {"Premier League": ["Man. City", "Arsenal", "Liverpool"], "La Liga": ["Real Madrid", "Barcelona"]}}
}
...
if st.session_state.aba_ativa == "home":
    ...
    with h2: draw_card("IA ASSERTIVIDADE", "92%", 92)
    with h3: draw_card("STAKE SUGERIDA", f"{st.session_state.stake_padrao}%", 100)
    with h4: draw_card("RISCO", "MODERADO", 70)
    ...
    # BOTÃO COM EFEITO APLICADO VIA CSS
    ...
    nova_banca = st.number_input("VALOR TOTAL DA SUA BANCA", value=st.session_state.banca_total)
    ...
    for call in reversed(st.session_state.historico_calls):
        st.markdown(f"""<div class="history-card-box">[{call['data']}] {call['casa']} x {call['fora']} | {call['stake_val']}</div>""", unsafe_allow_html=True)
