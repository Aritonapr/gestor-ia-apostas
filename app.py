...
# ==============================================================================
# [PROTOCOLO DE MANUTENÇÃO v57.23 - RESTAURAÇÃO DE BASE SEGURA]
# ==============================================================================
...
# 2. CSS INTEGRAL E BLINDADO (ORIGINAL)
st.markdown("""
    <style>
...
# 3. SIDEBAR
with st.sidebar:
    st.markdown("""<div class="betano-header"><div class="header-left"><div class="logo-link">GESTOR IA</div></div><div class="header-right"><div class="entrar-grad">v57.23</div></div></div><div style="height:65px;"></div>""", unsafe_allow_html=True) 
...
# TELA 2: GESTÃO
    ...
    with col_display:
        ...
        with g3: draw_card("STOP LOSS", "5%", 100)
        with g4: draw_card("SAÚDE", "ESTÁVEL", 100, "#00ff88")
        ...
        with g8: draw_card("IA MODO", "PRO", 100)

# TELA 3: SCANNER (PREENCHIMENTO DE TIMES SEGURO)
elif st.session_state.aba_ativa == "analise":
    st.markdown("<h2 style='color:white;'>🎯 SCANNER PRÉ-LIVE</h2>", unsafe_allow_html=True)
    
    # Listas diretas para evitar erro de dicionário
    lista_paises = ["BRASIL", "INGLATERRA", "ESPANHA", "ALEMANHA", "ITÁLIA"]
    lista_brasileirao = ["Botafogo", "Palmeiras", "Flamengo", "Fortaleza", "São Paulo", "Internacional", "Cruzeiro", "Bahia", "Vasco", "Atlético-MG"]
    
    cat = st.selectbox("🌎 CATEGORIA", lista_paises)
    tip = st.selectbox("📂 COMPETIÇÃO", ["Série A", "Copa Nacional", "Finais"])
    
    c1, c2 = st.columns(2)
    t_casa = c1.selectbox("🏠 MANDANTE", lista_brasileirao, index=0)
    t_fora = c2.selectbox("🚀 VISITANTE", lista_brasileirao, index=1)
    
    if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True):
        v_calc = (st.session_state.banca_total * st.session_state.stake_padrao / 100)
        st.session_state.analise_bloqueada = {"casa": t_casa, "fora": t_fora, "vencedor": t_casa, "gols": "OVER 2.5", "data": datetime.now().strftime("%H:%M"), "stake_val": f"R$ {v_calc:,.2f}"}
...
# TELAS DE ANÁLISE (PADRÃO 8 CARDS)
    ...
    with l5: draw_card("ODD", "1.85", 100)
    with l6: draw_card("VARIAÇÃO", "+0.12", 40)
    with l7: draw_card("CORNERS", "8", 80)
    with l8: draw_card("STAKE", "1%", 100)
    ...
    g5, g6, g7, g8 = st.columns(4)
    with g5: draw_card("OVER 2.5", "54%", 54)
    with g6: draw_card("GOL 2T", "70%", 70)
    with g7: draw_card("MÉDIA", "2.8", 100)
    ...
    with e3: draw_card("HT 4.5", "70%", 70)
    with e4: draw_card("RACE 5", "Time A", 100)
    e5, e6, e7, e8 = st.columns(4)
    with e5: draw_card("ASIÁTICO", "9.0", 100)
    with e6: draw_card("MÉDIA", "10.2", 100)
    with e7: draw_card("CONF.", "ALTA", 100)
    with e8: draw_card("INDICADOR", "CANTOS", 100)
