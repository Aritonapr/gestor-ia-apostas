...
# ==============================================================================
# [PROTOCOLO DE MANUTENÇÃO v58.9 - RESTAURAÇÃO TOTAL + FIX BOTS]
# DIRETRIZ: CORREÇÃO DO KEYERROR + RESTAURAÇÃO DAS ABAS DE GESTÃO E MERCADOS
# ==============================================================================
...
# [LÓGICA IA] - DADOS INTEGRADOS DOS BOTS 1, 2 E 3
...
st.markdown("""
    ...
    * { -ms-overflow-style: none !important; scrollbar-width: none !important; }
    ...
    div.stButton > button:not([data-testid="stSidebar"] *) { background: linear-gradient(90deg, #6d28d9 0%, #06b6d4 100%) !important; color: white !important; font-weight: 900 !important; text-transform: uppercase !important; border-radius: 6px !important; width: 100% !important; border: none !important; padding: 15px !important; }
...
if st.session_state.aba_ativa == "home":
    st.markdown("<h2 style='color:white;'>📅 BILHETE OURO (IA ANALIST)</h2>", unsafe_allow_html=True)
    ...
    with h4: draw_card("VENCEDOR PROB.", st.session_state.dados_ia["vencedor"], 100)
    ...
    st.markdown("<h2 style='color:white;'>📡 SCANNER LIVE (BOT 2)</h2>", unsafe_allow_html=True)
    l1, l2, l3, l4 = st.columns(4)
    with l1: draw_card("PRESSÃO IA", "85%", 85)
    with l2: draw_card("GOLS PROVÁVEIS", st.session_state.dados_ia["gols"], 70)
    with l3: draw_card("TIROS DE META", st.session_state.dados_ia["tiros_meta"], 65) # FIX KEYERROR
    with l4: draw_card("DEFESAS GOLEIRO", st.session_state.dados_ia["defesas"], 90)

elif st.session_state.aba_ativa == "analise":
    st.markdown("<h2 style='color:white;'>🎯 SCANNER PRÉ-LIVE</h2>", unsafe_allow_html=True)
    st.info("Sua seleção de times manual está ativa.")
    t1, t2 = st.columns(2)
    with t1: st.selectbox("TIME CASA", ["Flamengo", "Palmeiras", "Real Madrid"])
    with t2: st.selectbox("TIME FORA", ["Vasco", "Corinthians", "Barcelona"])
    if st.button("EXECUTAR ANÁLISE IA"): st.success("Análise Concluída!")
...
st.markdown(f"""<div class="footer-shield"><div>STATUS: ● IA OPERACIONAL | v58.9 | SYNC: {st.session_state.dados_ia["sync"]}</div><div>JARVIS PROTECT</div></div>""", unsafe_allow_html=True)
