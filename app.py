...
# ==============================================================================
# [PROTOCOLO v57.25 - LEGIBILIDADE MÁXIMA + INTEGRIDADE TOTAL]
# ==============================================================================
...
# 2. CSS INTEGRAL (FOCO EM LEITURA DO MENU E ZERO SCROLL)
st.markdown("""
    <style>
    ...
    * { -ms-overflow-style: none !important; scrollbar-width: none !important; }
    ...
    /* HEADER PREMIUM - AJUSTE DE CONTRASTE */
    .betano-header { 
        position: fixed; top: 0; left: 0; width: 100%; height: 60px; 
        background-color: #001a4d !important; border-bottom: 1px solid rgba(255,255,255,0.1) !important; 
    ...
    /* MENU SUPERIOR: FONTE 11PX, BRANCO PURO, SEM OPACIDADE BAIXA */
    .nav-item { 
    ...
    /* SIDEBAR NAVIGATION - FIXO SEM ROLAGEM */
...
# TELA: SCANNER
    ...
    c1, c2 = st.columns(2)
    t_casa = c1.selectbox("🏠 MANDANTE", ["Botafogo", "Palmeiras", "Flamengo", "Fortaleza"])
    t_fora = c2.selectbox("🚀 VISITANTE", ["Vasco", "São Paulo", "Atlético-MG", "Cruzeiro"])
...
# TELAS DE ANÁLISE (PADRÃO 8 CARDS)
    ...
    l1, l2, l3, l4 = st.columns(4); [draw_card(f"LIVE {i}", "---", 50) for i in range(1, 5)]
    l5, l6, l7, l8 = st.columns(4); [draw_card(f"LIVE {i}", "---", 50) for i in range(5, 9)]

elif st.session_state.aba_ativa == "gols":
    st.markdown("<h2 style='color:white;'>⚽ APOSTAS POR GOLS</h2>", unsafe_allow_html=True)
    g1, g2, g3, g4 = st.columns(4); [draw_card(f"GOLS {i}", "---", 50) for i in range(1, 5)]
    g5, g6, g7, g8 = st.columns(4); [draw_card(f"GOLS {i}", "---", 50) for i in range(5, 9)]
    ...
    e1, e2, e3, e4 = st.columns(4); [draw_card(f"CANTOS {i}", "---", 50) for i in range(1, 5)]
    e5, e6, e7, e8 = st.columns(4); [draw_card(f"CANTOS {i}", "---", 50) for i in range(5, 9)]

elif st.session_state.aba_ativa == "vencedores":
    st.markdown("<h2 style='color:white;'>🏆 VENCEDORES</h2>", unsafe_allow_html=True)
    v1, v2, v3, v4 = st.columns(4); [draw_card(f"WIN {i}", "---", 50) for i in range(1, 5)]
    v5, v6, v7, v8 = st.columns(4); [draw_card(f"WIN {i}", "---", 50) for i in range(5, 9)]
