...
# --- BANCO DE DADOS DE TIMES (SINCRONIZAÇÃO COMPLETA) ---
# Aqui mapeamos os times para cada competição individualmente
times_db = {
    # BRASIL
    ...
    "Copa do Brasil": ["Flamengo", "São Paulo", "Palmeiras", "Corinthians", "Atlético-MG", "Vasco", "Bahia", "Juventude"],
    "Paulistão": ["Palmeiras", "Santos", "São Paulo", "Corinthians", "Red Bull Bragantino", "Novorizontino", "Inter de Limeira"],
    "Carioca": ["Flamengo", "Fluminense", "Botafogo", "Vasco", "Nova Iguaçu", "Boavista"],
    
    # EUROPEIA
    "Premier League": ["Man City", "Arsenal", "Liverpool", "Aston Villa", "Tottenham", "Chelsea", "Man United", "Newcastle", "Brighton", "West Ham", "Everton", "Leicester"],
    "La Liga": ["Real Madrid", "Barcelona", "Atlético de Madrid", "Girona", "Athletic Bilbao", "Real Sociedad", "Real Betis", "Villarreal", "Sevilla", "Valencia"],
    "Bundesliga": ["Bayer Leverkusen", "Bayern Munich", "Dortmund", "RB Leipzig", "Stuttgart", "Frankfurt"],
    "Serie A (Itália)": ["Inter de Milão", "Milan", "Juventus", "Atalanta", "Napoli", "Roma", "Lazio"],
    
    # INTERNACIONAIS (RESTAURADOS)
    "Champions League": ["Real Madrid", "Man City", "Bayern Munich", "Arsenal", "Barcelona", "PSG", "Inter de Milão", "Dortmund", "Liverpool", "Bayer Leverkusen", "Atletico Madrid", "Juventus", "Benfica", "Sporting", "Milan"],
    "Europa League": ["Man United", "Tottenham", "Roma", "Porto", "Ajax", "Lazio", "Real Sociedad", "Lyon", "Fenerbahçe"],
    ...
    "Copa Sul-Americana": ["Cruzeiro", "Corinthians", "Fortaleza", "Athletico-PR", "Internacional", "Boca Juniors", "Lanús", "Racing", "Cuiabá"],
    "Mundial de Clubes FIFA": ["Real Madrid", "Man City", "Flamengo", "Palmeiras", "Fluminense", "Chelsea", "Bayern Munich", "Al-Hilal"]
}
...
db_global = {
    ...
    "🏆 TORNEIOS INTERNACIONAIS": {
        ...
        "Continente (Clubes BR)": ["Copa Libertadores", "Copa Sul-Americana", "Recopa"],
...
if st.session_state.app_state == "processar":
    ...
    # Busca os times da lista times_db. Se não existir, mostra uma lista genérica.
    elenco_final = times_db.get(comp_sel, ["Time A", "Time B", "Time C"])
    
    t1, t2 = st.columns(2)
    with t1: casa = st.selectbox("TIME CASA", elenco_final, index=0)
    with t2: fora = st.selectbox("TIME FORA", [t for t in elenco_final if t != casa], index=0)
...
st.markdown("""<div class="betano-footer"><div>STATUS: ● IA OPERACIONAL | ELENCOS: SINCRONIZADOS</div><div>GESTOR IA PRO v3.0 | 18+ JOGUE COM RESPONSABILIDADE</div></div>""", unsafe_allow_html=True)
