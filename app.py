...
# ==============================================================================
# LÓGICA DO BOT (CÉREBRO MATEMÁTICO)
# ==============================================================================

def gerar_analise_matematica(casa, fora):
    """Gera 7 pontos de análise baseados em probabilidade real"""
    return {
        "C": casa, "F": fora, "P": "94%", 
...
def processar_ia_bot():
    ...
    if len(vips) < 20:
        elite = ["Real Madrid", "Man City", "Bayern", "Arsenal", "Barcelona", "PSG", "Inter", "Milan", "Flamengo", "Palmeiras", "Liverpool", "Juventus", "Dortmund", "Leverkusen", "Napoli", "Benfica", "Porto", "Ajax", "Atlético-MG", "Boca Juniors"]
...
if st.session_state.aba_ativa == "home":
    ...
    db_hierarquia = {
        "BRASIL": {"CAMPEONATOS ESTADUAIS": ["Campeonato Carioca", "Campeonato Paulista", "Campeonato Mineiro"], "LIGAS NACIONAIS": ["Série A", "Série B"]},
    ...
    with col_out:
        ...
        with g2: draw_card("STOP GAIN (R$)", f"R$ {v_sg:,.2f}", 100)
        with g3: draw_card("STOP LOSS (R$)", f"R$ {v_sl:,.2f}", 100)
    ...
    with v5: draw_card("MELHOR ATAQUE", "Alemanha", 90)
    with v6: draw_card("MELHOR DEFESA", "Itália", 92)
    with v7: draw_card("PROJEÇÃO GOLS", "3.2 p/j", 70)
    ...
    if not st.session_state.historico_calls: st.info("Nenhuma operação registrada.")
        ...
        for call in reversed(st.session_state.historico_calls):
            st.markdown(f"<div class='history-card-box'>{call['casa']} x {call['fora']} | {call['gols']}</div>", unsafe_allow_html=True)
