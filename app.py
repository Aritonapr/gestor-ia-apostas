...
# ==============================================================================
# [PROTOCOLO DE MANUTENÇÃO v63.3 - RESTAURAÇÃO TOTAL + 8 CARDS GESTÃO]
# ==============================================================================
...
def processar_ia_bot():
    ...
    for i in range(20):
        vips.append({
            ...
            "V": "72% (FAVORITO)", "G": "1.5+ GOLS", "CT": "4.5 total",
            "E": "9.5 total", "TM": "14+ total", "CH": "9+ total", "DF": "7+ total"
        })
    st.session_state.top_20_ia = vips

def executar_scanner_live():
    times_live = [("Liverpool", "Everton"), ("Real Madrid", "Sevilla"), ("Napoli", "Lazio"), ("Boca", "River"), ("Palmeiras", "Santos"), ("Benfica", "Porto"), ("PSG", "Lyon"), ("Arsenal", "Spurs"), ("Bayern", "Leipzig"), ("Inter", "Roma"), ("City", "United"), ("Galo", "Cruzeiro"), ("Chelsea", "Vasco"), ("Milan", "Inter"), ("Ajax", "PSV"), ("Bayer", "Dortmund"), ("Atletico", "Getafe"), ("Porto", "Benfica"), ("Flamengo", "Gremio"), ("Cruzeiro", "Galo")]
    novos = []
    for i in range(20):
...
if not st.session_state.jogos_live_ia: executar_scanner_live()
...
st.markdown("""
    ...
    .kpi-detailed-card { background: #11151a; border: 1px solid #1e293b; padding: 20px 18px; border-radius: 8px; margin-bottom: 15px; height: auto !important; transition: 0.3s ease; }
...
with st.sidebar:
    st.markdown("""
        <div class="betano-header">
            <div style="display:flex; align-items:center;"><a href="#" class="logo-link">GESTOR IA</a><div class="nav-item">ESPORTES</div><div class="nav-item">LIVE</div></div>
            <div style="display:flex; align-items:center; gap:10px;"><div class="registrar-pill">REGISTRAR</div><div class="entrar-grad">ENTRAR</div></div>
        </div>
...
if st.session_state.aba_ativa == "home":
    ...
    render_20_cards(st.session_state.top_20_ia, "IA CONFIANÇA:", "#9d54ff")

elif st.session_state.aba_ativa == "analise":
    st.markdown("<h2 style='color:white;'>🎯 SCANNER PRÉ-LIVE</h2>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1: st.selectbox("🌎 REGIÃO / PAÍS", ["BRASIL", "EUROPA", "AMÉRICAS"])
    with c2: st.selectbox("📂 GRUPO", ["LIGAS NACIONAIS", "COPAS", "ELITE"])
    with c3: st.selectbox("🏆 COMPETIÇÃO", ["SÉRIE A", "PREMIER LEAGUE", "CHAMPIONS"])
    t1, t2 = st.columns(2)
    with t1: tc = st.selectbox("🏠 TIME DA CASA", ["Flamengo", "Palmeiras", "Real Madrid"])
    with t2: tf = st.selectbox("🚀 TIME DE FORA", ["Corinthians", "Galo", "Man City"])
    if st.button("⚡ EXECUTAR ALGORITIMO"):
        v_calc = (st.session_state.banca_total * st.session_state.stake_padrao / 100)
        st.session_state.analise_bloqueada = {"casa": tc, "fora": tf, "vencedor": "ALTA PROB.", "gols": "OVER 1.5", "stake_val": f"R$ {v_calc:,.2f}", "cantos": "9.5+", "btss": "SIM (74%)", "cartoes": "4.5+", "chutes": "8.5 p/g", "confia": "94.2%", "data": datetime.now().strftime("%H:%M"), "luz": "🟢", "motivo": "INFORMAÇÃO REAL", "cor": "#00ff88"}
    if st.session_state.analise_bloqueada:
        m = st.session_state.analise_bloqueada
        st.markdown(f"<div style='background:rgba(255,255,255,0.03); border-left:5px solid {m['cor']}; padding:18px; border-radius:6px; margin-top:25px;'><b>SISTEMA JARVIS:</b> <span style='color:{m['cor']};'>{m['motivo']}</span></div>", unsafe_allow_html=True)
        ...
        if st.button("📥 SALVAR CALL"): st.session_state.historico_calls.append(m.copy()); st.toast("✅ SALVO!")
    ...
    with c_out:
        ...
        with l1_1: draw_card("VALOR ENTRADA", f"R$ {v_s:,.2f}", 100, "#00d2ff")
        with l1_2: draw_card("STOP GAIN", f"R$ {v_m:,.2f}", 100, "#00d2ff")
        with l1_3: draw_card("STOP LOSS", f"R$ {v_l:,.2f}", 100, "#00d2ff")
        with l1_4: draw_card("ALVO FINAL", f"R$ {st.session_state.banca_total + v_m:,.2f}", 100, "#00d2ff")
        l2_1, l2_2, l2_3, l2_4 = st.columns(4)
        with l2_1: draw_card("RISCO TOTAL", f"{st.session_state.stake_padrao}%", 100, "#6d28d9")
        with l2_2: draw_card("ENTRADAS/META", f"{int(v_m/v_s) if v_s > 0 else 0}", 100, "#6d28d9")
        with l2_3: draw_card("ENTRADAS/LOSS", f"{int(v_l/v_s) if v_s > 0 else 0}", 100, "#6d28d9")
        with l2_4: draw_card("SAÚDE BANCA", "EXCELENTE", 100, "#00ff88")

elif st.session_state.aba_ativa == "live":
    st.markdown("<h2 style='color:white; margin-bottom:30px;'>📡 SCANNER EM TEMPO REAL (TOP 20 LIVE)</h2>", unsafe_allow_html=True)
    render_20_cards(st.session_state.jogos_live_ia, "IA LIVE:", "#00ff88")

elif st.session_state.aba_ativa == "vencedores":
    st.markdown("<h2 style='color:white; margin-bottom:30px;'>🏆 VENCEDORES DA COMPETIÇÃO - TOP 20</h2>", unsafe_allow_html=True)
    render_20_cards(st.session_state.top_20_ia, "CHANCE VITÓRIA:", "#ffcc00")
    ...
    render_20_cards(st.session_state.top_20_ia, "PROB. GOLS:", "#00d2ff")
    ...
    render_20_cards(st.session_state.top_20_ia, "PROB. CANTOS:", "#ff4b4b")
    ...
    if not st.session_state.historico_calls: st.info("Nenhuma operação registrada.")
        ...
        for row in reversed(rows):
            ...
            for i, c in enumerate(row):
                with cols[i]:
                    st.markdown(f"""<div class="kpi-detailed-card"><div style="color:#9d54ff; font-size:10px; font-weight:900; margin-bottom:5px;">DATA: {c['data']}</div><div style="color:white; font-size:12px; font-weight:800; margin-bottom:12px; border-bottom:1px solid #1e293b; padding-bottom:5px;">{c['casa']} vs {c['fora']}</div><div class="kpi-stat">🏆 VENCEDOR: <b>{c['vencedor']}</b></div><div class="kpi-stat">⚽ GOLS: <b>{c['gols']}</b></div><div class="kpi-stat">🟨 CARTÕES: <b>{c['cartoes']}</b></div><div class="kpi-stat">🚩 ESCANTEIOS: <b>{c['cantos']}</b></div><div class="kpi-stat">👟 TIROS META: <b>{c['chutes']}</b></div><div class="kpi-stat">🥅 CHUTES GOL: <b>{c['btss']}</b></div><div style="margin-top:15px; padding-top:12px; border-top:1px dashed #334155; color:#06b6d4; font-size:11px; font-weight:900; text-align:center;">STAKE SALVA: {c['stake_val']}</div></div>""", unsafe_allow_html=True)

st.markdown("""<div class="footer-shield"><div>STATUS: ● IA OPERACIONAL | v63.3</div><div>JARVIS PROTECT</div></div>""", unsafe_allow_html=True)
