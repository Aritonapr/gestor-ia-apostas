...
# TELA 3: SCANNER PRÉ-LIVE (FILTROS COMPLETOS)
    ...
    with c1: categoria_sel = st.selectbox("🌎 CATEGORIA", list(db_gestor.keys()))
    with c2: tipo_sel = st.selectbox("📂 TIPO", list(db_gestor[categoria_sel].keys()))
    with c3: campeonato_sel = st.selectbox("🏆 CAMPEONATO", list(db_gestor[categoria_sel][tipo_sel].keys()))

    lista_times = db_gestor[categoria_sel][tipo_sel][campeonato_sel]

    # SELEÇÃO DE TIMES
    m1, m2 = st.columns(2)
    with m1: casa = st.selectbox("🏠 MANDANTE", lista_times, index=0)
    with m2: 
        idx_v = 1 if len(lista_times) > 1 else 0
        fora = st.selectbox("🚀 VISITANTE", lista_times, index=idx_v)
        ...
        if casa == fora:
            ...
            with st.spinner("🧠 PROCESSANDO..."):
                ...
                st.session_state.analise_bloqueada = {
                    "casa": casa, "fora": fora, "vencedor": "Casa", "gols": "OVER 1.5", 
