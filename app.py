...
# ==============================================================================
# [GIAE KERNEL SHIELD v57.26 - FINAL STABILIZATION]
# FIX: HEADER OVERLAP | SEARCH ICON RESTORED | BANKROLL SYNC
# INTEGRITY: FULL CODE | NO ABBREVIATIONS | 8 CARDS GRID | 7 SIDEBAR BUTTONS
# ==============================================================================
...
def draw_card(title, value, perc):
    """Renderiza os quadros de alta performance conforme padrão visual Anderson."""
    st.markdown(f"""
...
st.markdown("""
    ...
    /* CABEÇALHO (NAVBAR) - FIX PARA SOBREPOSIÇÃO */
    ...
    .logo-link { color: #9d54ff !important; font-weight: 900; font-size: 20px !important; text-transform: uppercase; letter-spacing: 1px; text-decoration: none !important; margin-right: 20px; cursor: pointer !important; }
    .nav-items { display: flex; gap: 15px; align-items: center; }
    .nav-items span { color: #ffffff; font-size: 8px !important; text-transform: uppercase; opacity: 0.7; white-space: nowrap; cursor: pointer !important; font-weight: 600; }
    
    .header-right { display: flex; align-items: center; gap: 15px; min-width: 300px; justify-content: flex-end; }
    .search-icon { color: #ffffff; font-size: 14px; cursor: pointer !important; opacity: 0.8; margin-right: 5px; }
    .registrar-pill { color: #ffffff !important; font-size: 10px !important; font-weight: 700; border: 1px solid #ffffff !important; padding: 6px 15px !important; border-radius: 20px !important; transition: 0.3s; cursor: pointer !important; white-space: nowrap; }
...
# --- [BASE DE DADOS EXPANDIDA - IMAGEM 2] ---
DADOS_HIEARARQUIA = {
    "🏆 COPA DO MUNDO 2026": {"Seleções FIFA": {"Principais": ["Brasil", "Argentina", "França", "Alemanha", "Portugal", "Espanha", "Inglaterra"]}},
    "🇧🇷 BRASIL (LIGAS & COPAS)": {"Campeonato Brasileiro": {"Série A": ["Flamengo", "Palmeiras", "Botafogo", "Vasco", "São Paulo", "Atlético-MG"], "Série B": ["Santos", "Sport", "Goiás"], "Série C": ["Remo", "Náutico"], "Série D": ["Santa Cruz", "Treze"]}},
...
# --- [ABA: SCANNER - 8 CARDS + ANTI-FLICKER] ---
    ...
    def area_scanner_integrada():
        ...
        if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True):
            # CÁLCULO DE STAKE REAL BASEADO NA GESTÃO
            valor_em_reais = (st.session_state.banca_total * st.session_state.stake_padrao / 100)
            st.session_state.analise_bloqueada = {
                "casa": casa, "fora": fora, "vencedor": casa, "gols": "OVER 1.5 REAL", 
                "data": datetime.now().strftime("%H:%M"), "stake_calc": f"R$ {valor_em_reais:,.2f}"
            }
    ...
    area_scanner_integrada()

# --- [ABA: SCANNER LIVE - DESIGN GRID] ---
    ...
    st.markdown("""<div class="news-ticker">● MONITORANDO JOGOS AO VIVO ● VARREDURA DE PRESSÃO IA ATIVA</div>""", unsafe_allow_html=True)
    ...
    with l5: draw_card("CANTOS HT", "6", 40)
    with l6: draw_card("CHUTES GOL", "12", 50)
    with l7: draw_card("DEFESAS", "4", 30)
    with l8: draw_card("PRESSÃO IA", "MÁXIMA", 100)

# --- [ABA: GESTÃO - DESIGN IMAGEM 1] ---
    ...
    g1, g2 = st.columns(2)
    with g1:
        nova_b = st.number_input("DIGITE O VALOR DA SUA BANCA (R$)", value=st.session_state.banca_total, step=100.0)
    ...
    with g2:
        ...
        st.info(f"Sua Stake padrão será de: R$ {calc_reais:,.2f}")
...
st.markdown("""<div class="footer-shield"><div>STATUS: ● IA OPERACIONAL | v57.26 LOCKED</div><div>JARVIS PROTECT</div></div>""", unsafe_allow_html=True)
