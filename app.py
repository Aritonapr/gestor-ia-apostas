...
# ==============================================================================
# 4. LÓGICA DE TELAS (RESTAURAÇÃO INTEGRAL)
# ==============================================================================
...
if st.session_state.aba_ativa == "home":
    ...
    row_f = st.columns(3)
    with row_f[0]: sel_regiao = st.selectbox("🌎 REGIÃO / PAÍS", list(db_hierarquia.keys()))
    with row_f[1]: sel_grupo = st.selectbox("📂 GRUPO", list(db_hierarquia[sel_regiao].keys()))
    with row_f[2]: sel_comp = st.selectbox("🏆 COMPETIÇÃO", db_hierarquia[sel_regiao][sel_grupo])
    ...
    if not lista_base: lista_base = ["Flamengo", "Palmeiras", "Real Madrid", "Man City", "Arsenal", "Barcelona"]
    ...
    if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True):
        ...
        st.session_state.analise_bloqueada = {
            ...
            "chutes": "8.5 p/g", "confia": "94.2%", "luz": "🟢", "motivo": "BIG DATA REAL", "cor": "#00ff88", "data": datetime.now().strftime("%H:%M")
        }
    
    if st.session_state.analise_bloqueada:
        m = st.session_state.analise_bloqueada
        st.markdown(f"<h3 style='color:white; text-align:center; font-weight:800; margin-bottom:25px;'>{m['casa']} vs {m['fora']}</h3>", unsafe_allow_html=True)
