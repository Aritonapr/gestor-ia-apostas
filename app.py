...
# ==============================================================================
# [PROTOCOLO DE MANUTENÇÃO v58.50 - PERSISTÊNCIA & ASSERTIVIDADE]
...
# --- FUNÇÕES DE MEMÓRIA PERMANENTE (SISTEMA DE ARQUIVOS) ---
...
if 'historico_calls' not in st.session_state: 
    st.session_state.historico_calls = carregar_historico_permanente()
...
# --- FUNÇÃO DE CARREGAMENTO DE DADOS DIÁRIOS ---
...
if st.session_state.aba_ativa == "home":
    ...
    with h2: draw_card("ASSERTIVIDADE", "92.4%", 92, "#00ff88")
    ...
    if st.button("⚡ EXECUTAR ALGORITIMO", use_container_width=True):
        ...
        if df_diario is not None:
            ...
            if not match.empty:
                status_luz, cor_luz, validacao_txt = ("🟢", "#00ff88", "FILÉ MIGNON: INFORMAÇÃO REAL E ATUALIZADA")
                conf_ia = "94.2%"
                res_venc = "ALTA PROB."
                res_gols = "OVER 1.5"

        st.session_state.analise_bloqueada = {
            "casa": t_casa, "fora": t_fora, "vencedor": res_venc, "gols": res_gols,
            "data": datetime.now().strftime("%d/%m %H:%M"), "stake_val": f"R$ {v_calc:,.2f}",
            "luz": status_luz, "cor": cor_luz, "motivo": validacao_txt, "confia": conf_ia
        }
    ...
    if st.session_state.analise_bloqueada:
        ...
        if st.button("📥 SALVAR CALL NO HISTÓRICO PERMANENTE", use_container_width=True):
            salvar_call_permanente(m)
            st.session_state.historico_calls = carregar_historico_permanente()
            st.toast("✅ SALVO NO GITHUB COM SUCESSO!")

elif st.session_state.aba_ativa == "historico":
    st.markdown("<h2 style='color:white;'>📜 HISTÓRICO PERMANENTE (MEMÓRIA IA)</h2>", unsafe_allow_html=True)
